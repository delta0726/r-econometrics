...
JS_did <-
  JS_sum %>%
    ...
    lm(death ~ LSV + D1854 + D1854:LSV, data = .) %>%
    tidy()
...
JS_did_log <-
  JS_sum %>%
    mutate(D1854 = if_else(year == 1854, 1, 0)) %>%
    lm(log(death) ~ LSV + D1854 + D1854:LSV, data = .) %>%
    tidy()
...
JS_did_area <-
  JS_df %>%
    mutate(D1854 = if_else(year == 1854, 1, 0)) %>%
    lm(death ~ LSV + area + D1854 + D1854:LSV, data = .) %>%
    tidy() %>%
...
JS_did_area_log <-
  JS_df %>%
    mutate(D1854 = if_else(year == 1854, 1, 0)) %>%
    lm(log(death) ~ LSV + area + D1854 + D1854:LSV, data = .) %>%
    tidy() %>%
    filter(!str_detect(term, "area"))
...
Cigar_did_sum_reg <-
  Cigar_did_sum %>%
    lm(sales ~ ca + post + ca:post + year_dummy, data = .) %>%
    tidy() %>%
...
Cigar_did_sum_logreg <-
  Cigar_did_sum %>%
    lm(log(sales) ~ ca + post + ca:post + year_dummy, data = .) %>%
    tidy() %>%
    filter(!str_detect(term, "state"),
           !str_detect(term, "year"))
